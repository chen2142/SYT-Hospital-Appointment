<template>
    ...
    <el-card class="box-card">
        ...
        <div class="visitors" v-if="scene == 0">
            <Visitor @changeScene="changeScene" @removeUser="removeUser" class="item" v-for="(user, index) in userArr" :key="user.id" :user="user"
                :index="index"></Visitor>
        </div>
        <!-- 添加就诊人|修改已有就诊人信息的结构 -->
        <div class="form" v-if="scene == 1">
            ...
            <el-form style="width: 60%;margin: 10px auto;">
                ...
                <el-form-item label="证件类型">
                    <el-select placeholder="请你选择证件的类型" v-model="userParams.certificatesType">
                ...
                <el-form-item label="出生日期">
                    <el-date-picker type="date" placeholder="请你选择一个日期" v-model="userParams.birthdate"
            ...
            <el-form style="width: 60%;margin: 10px auto;" label-width="80">
                ...
                <el-form-item label="证件类型">
                    <el-select placeholder="请你选择证件的类型" v-model="userParams.contactsCertificatesType">
                        <el-option v-for="item in certationArr" :key="item.id" :label="item.name"
                            :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
...
import type { CertificateTypeResponseData, CertificateArr, AddOrUpdateUser } from '@/api/user/type.ts'
// 引入路由器与路由方法
...
// 存储证件类型的数据
let certationArr = ref<CertificateArr>([])
// 收集新增就诊人的数据
...
onMounted(() => {
    ...
    // 判断：当前的路由组件是不是从挂号组件而来[挂号组件而来，路径上是携带query参数type=add]
