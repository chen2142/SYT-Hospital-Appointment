<template>
    ...
    <el-card class="box-card">
        ...
        <!-- 用户未验证的结构 -->
        <el-form v-if="userInfo.authStatus == 0" style="width: 60%;margin: 20px auto;" label-width="80" :model="params" :rules="rules" ref="form">
            ...
            <el-form-item label="上传证件" prop="certificatesUrl">
                ...
                <!-- limit:照片墙约束图片个数 -->
                <!-- on-exceed:超出约束数量的钩子 -->
                <el-upload ref="upload" list-type="picture-card" :limit="1" :on-exceed="exceedhandler"
                    ...
                    <img style="width: 100%;height: 100%;" src="@/assets/images/autu_example.png" alt="">
                </el-upload>
                <el-dialog v-model="dialogVisible">
                    <img style="width: 100%;" v-if="params.certificatesUrl" :src="params.certificatesUrl" w-full
                        alt="Preview Image" />
                </el-dialog>
            </el-form-item>
            <el-form-item>
...
import type { CertificateArr, CertificateTypeResponseData, UserInfo, UserInfoResponseData, UserParams } from '@/api/user/type';
...
// 获取证件类型的数据
const getType = async () => {
    let result: CertificateTypeResponseData = await reqCertificationType()
    if (result.code == 200) {
...
// 图片上传成功的钩子
const successhandler = (response: any, uploadFile: any) => {
    // 如果图片上传成功，校验结果清除
...
const submit = async () => {
    ...
    // 如果有一个表单校验失败返回的是一个失败的promise对象，后面的语句就不再执行
