...
import type { AddOrUpdateUser, AllOrderStateResponseData, AllUserResponseData, CertificateTypeResponseData, OrderResponseData,PayResult,QrCode,ResponseData,SubmitOrder, UserInfoResponseData, UserOrderInfoResponseData, UserParams } from './type';
// 枚举地址
enum API{
    //提交订单，获取订单号码接口
...
// 获取订单详情的方法
export const reqOrderInfo = (id:string) => request.get<AnalyserNode,OrderResponseData>(API.GETORDERINFO_URL+id)
...
// 获取证件类型的方法
export const reqCertificationType = (CertificatesType = 'CertificatesType')=>request.get<any,CertificateTypeResponseData>(API.CERTIFICATIONTYPE_URL+CertificatesType)
